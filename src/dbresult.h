...
///////////////////////////////////////////////////////////////////////////////
/// A row of columns.
class Record : public std::vector<std::experimental::any>
{
public:
    template <typename T> T get(size_type pos) const
    {
        return std::experimental::any_cast<T>(operator[](pos));
    }
...
public:
    ...
    //------------------------------------------------------------------------
    void clear() { m_field_spec.clear();  m_field_by_name.clear(); m_row.clear(); }
    ...
    void emplace_back(Args&&... args) { m_row.emplace_back(std::forward<Args>(args)...); }
...
public:
    ...
    void add_result(const std::vector<pg::FieldSpec> &fs) { m_rset.emplace_back(fs); }
